                ...
                <table class="table align-items-center mb-0">
                  ...
                  <tbody>
                    ...
                    <x-product-list-card id="{{ $product->id }}" image="{{ $product->feature_image_path }}"
                      title="{{ $product->title }}" categoryId="{{ $product->category_id }}"
                      restockValue="{{ $product->restock_value }}" sellValue="{{$product->sell_value}}"
                      rate="{{$product->rate}}" view="{{$product->view}}" createdAt="{{$product->created_at}}"
      ...
      <div class="pro-pagination-style text-center mt-10">
        ...
        <ul>
          ...
          @for ($i = 1; $i <= $pages; $i++) <li>
            <a href='{{ "http://127.0.0.1:8000/admin/products?page=".$i }}'>{{ $i }}</a>
            </li>
            @endfor
